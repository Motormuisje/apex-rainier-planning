...
from modules.models import LineType, PlanningRow, ValuationParameters
...
def _planning_row_from_snapshot(snap, fallback_line_type=''):
...
def _build_pending_edits_from_results_snapshot(results_snapshot) -> dict:
...
def _rebuild_volume_caches_from_results(current_engine):
    ...
    if machines:
        for mc_code, machine in machines.items():
            ...
            machines_snapshot[mc_code] = {
                ...
                'shift_hours_computed': SHIFT_HOURS_LOOKUP_FALLBACK(machine, engine.data),
            }
    return {
        'results': {
            lt: [copy.deepcopy(_row_payload(r)) for r in rows]
...
def _machine_overrides_from_engine(sess, engine) -> dict:
...
def _apply_machine_overrides(engine, machine_overrides: dict) -> bool:
    data = getattr(engine, 'data', None)
    machines = getattr(data, 'machines', None)
...
def _snapshot_has_manual_edits(snapshot: dict) -> bool:
...
def _engine_has_manual_edits(engine) -> bool:
...
def _restore_engine_state(engine, snapshot: dict) -> None:
    ...
    _rebuild_volume_caches_from_results(engine)

    vp_snap = snapshot.get('valuation_params')
    if vp_snap and getattr(engine, 'data', None) is not None:
        engine.data.valuation_params = _valuation_params_from_config(vp_snap)
        _global_config['valuation_params'] = {str(k): float(v) for k, v in vp_snap.items()}
    ...
    if pap_snap is not None and getattr(engine, 'data', None) is not None:
        ...
        _global_config['purchased_and_produced'] = _format_purchased_and_produced(pap_snap)
...
def _ensure_reset_baseline(sess, engine) -> None:
    ...
    if baseline is None or (_snapshot_has_manual_edits(baseline) and not _engine_has_manual_edits(engine)):
...
def _valuation_params_from_config(value) -> ValuationParameters:
    vp = value or {}
